...
import { useForm, ValidationResolver, UseFormOptions, FormContextValues } from "react-hook-form";
...
const resolver: <T extends Record<string, any> = Record<string, any>>
  (i18n, schema: Schema<T>) => ValidationResolver<T>
  = ({ t }: { readonly t: TranslateFunction }, schema) => (data) =>
...
export type UseYupForm =
  <F extends Record<string, any> = Record<string, any>>(options: UseFormOptions<F>) => FormContextValues<F>;

export function initializeYupForm({ i18n }) {
  return <F extends Record<string, any> = Record<string, any>>(
    { validationSchema, ...options }: UseFormOptions<F>
  ) => useForm({
    ...options,
